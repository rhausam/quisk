...
class QMainFrame(wx.Frame):
  ...
  def __init__(self, width, height):
    ...
    fp.close()
    title = 'Quisk Vector Network Analyzer ' + title[7:]
...
class CalibrateDialog(wx.Dialog):
  ...
  def OnTimer(self, event):
    ...
    if self.app.has_SetVNA:
      Hardware.SetVNA(key_down=0, vna_count=self.app.data_width)
    for b in self.btns:
      b.Enable(True)
...
class App(wx.App):
  ...
  def EnableButtons(self):
    if self.screen_name == 'Transmission':
      ...
      if self.reflection_short is not None or self.reflection_open is not None:
        self.btn_run.Enable(1)
      else:
        self.btn_run.Enable(0)
    else:		# Help
